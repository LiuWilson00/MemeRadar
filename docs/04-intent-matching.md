...
## 5. 延遲預算（2026-07-11 依 P2-3 實測修訂：文字 ≤ 15 秒、截圖 ≤ 22 秒）

| 階段 | 預算 | 實測（sonnet-5、thinking off、字數上限） |
|------|------|------|
| 截圖解析（僅截圖輸入） | ≤ 7s | ~5s（合成 LINE 截圖） |
| 意圖分析 | ≤ 8s | ~7.3s |
| embedding + 向量檢索（多策略） | ≤ 0.5s | ~0.1s |
| Rerank | ≤ 7s | ~5.9s（pool=6；低分候選省略理由） |
| 其他（組裝 / 傳輸） | ≤ 1s | — |

已套用的優化：線上路徑顯式 `thinking: disabled`（省 ~15s）、輸出字數上限（意圖省 ~4s）、低分候選不寫理由。實測 haiku-4-5 不比 sonnet-5 快（指令遵循較差→輸出更長）且品質略遜，維持 sonnet-5。原 8 秒總目標保留為優化方向：Console 串流 / 骨架屏遮蔽感知延遲、縮減策略數、必要時評估 Voyage rerank。
