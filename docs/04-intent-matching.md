...
## 5. 延遲預算（2026-07-12 依線上 log 修訂：文字 ≤ 15 秒、截圖 ≤ 22 秒）

線上 log（124 張庫、真實用戶）顯示延遲漲到 p50 19s / p90 27s，遠超預算。分階段
即時量測發現：**兩個接續的 LLM 呼叫吃掉 >99% 時間，瓶頸是輸出生成（prompt 快取
只省輸入成本、幫不到延遲）**。

| 階段 | 預算 | 實測 sonnet-5 | 實測 haiku-4-5（現況） |
|------|------|------|------|
| 截圖解析（僅截圖輸入） | ≤ 7s | ~5s | ~5s（仍 sonnet） |
| 意圖分析 | ≤ 8s | ~7.5s | **~3.7s** |
| embedding + 向量檢索（多策略） | ≤ 0.5s | ~0.4s | ~0.3s |
| Rerank | ≤ 7s | ~9s（pool=25） | **~7s** |
| 其他（組裝 / 傳輸） | ≤ 1s | — | — |
| **文字端到端** | ≤ 15s | ~16.5s | **~11s** |

已套用的優化：
- 線上路徑顯式 `thinking: disabled`（省 ~15s）、輸出字數上限、低分候選不寫理由。
- **2026-07 成本優化：意圖分析 + rerank 改用 haiku-4-5**（實測 A/B：top 推薦與
  sonnet 相同，意圖 −51%、rerank −22%、端到端 16.5→11s（−33%），成本約 sonnet
  的 1/3）。需更高品質時以 `model=` 覆寫回 sonnet-5。標註 / 截圖解析 / 對方梗圖
  理解維持 sonnet-5（品質敏感、非最熱路徑）。
- **分階段耗時落庫**（`recommendation_logs.timings`，migration 0007）：延遲可持續
  監控，不必再靠即時 API 抓。

後續優化方向（未做）：rerank pool 25→18（需驗品質，pool 12 會換掉 top1）、截圖
「解析 + 意圖」併為單次呼叫（省一整次 LLM）、Console 串流 / 骨架屏遮蔽感知延遲。
