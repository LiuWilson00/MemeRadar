...
- [ ] **P2-6** Console 主頁（React + Vite）：輸入區（含解析預覽編輯）、參數面板、結果卡片 + 👍👎、Debug 面板（05 文件 §2.1）— **L**（依賴 P2-4；UI 骨架可先行）
  - 驗收：05 文件 §5 全項
  - 進度：主頁全功能已實作 ✅（氣泡編輯器 + 範例載入 + 貼上解析、參數面板全參數 + /meta 梗圖包、結果卡片含分數拆解 / 策略徽章 / 👍👎+備註、Debug 面板含意圖 / 候選池表格 / 耗時、雷達掃描 loading、空結果診斷、敏感情境警示）；lib 層 vitest 6 項 + production build 通過；經 Vite 代理的完整 E2E（recommend 13.4s / 圖片 / 回饋落庫）通過 ✅；視覺與操作流暢度驗收 ⏳ 待團隊開 http://localhost:5173 確認；截圖上傳流程 ⏳ 待 P2-5
