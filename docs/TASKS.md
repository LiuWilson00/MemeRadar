...
- [ ] **P2-1** 意圖分析：對話 → 意圖 JSON（04 文件 §2.2），含敏感情境偵測與 prompt injection 防護 — **M**（依賴 P0-2）
  ...
  - 進度：已實作並通過測試 ✅（`memeradar/matching/intent.py`：策略名鎖 taxonomy enum、敏感情境**程式層**強制僅留安撫、定界消毒防 injection、CLI 供評查）；真 API 煙霧 3 組（指責 / 喪事敏感 / 低上下文）輸出皆符合設計，敏感組僅安撫策略 ✅；「20 組人工評查」⏳ 待團隊執行（CLI 已就緒）
- [x] **P2-2** 多路檢索 + 合併：每策略平行檢索、去重合併、參數化（04 文件 §2.3、§3）— **M**（依賴 P1-5, P2-1）
  - 驗收 ✅（`memeradar/matching/retrieval.py`：策略 query 單批 embed、每策略 Top-K + 過濾、跨策略合併取最高分並記錄命中策略與各策略分數、per-strategy 回收數供 debug 面板；全線真實煙霧：intent 4 策略 → 檢索排序語意正確）
...
- [ ] **P2-5** 截圖解析：VLM 氣泡解析 → 結構化對話（04 文件 §2.1），截圖不落庫 — **M**（依賴 P2-4）
  ...
  - 進度：已實作 ✅（`matching/screenshot.py` + `POST /parse-screenshot` 供 Console 解析後編修 + `/recommend` screenshot 輸入補實作取代 501 + Console 上傳接線；截圖僅記憶體處理不落庫）；合成 LINE 截圖真實解析 5/5 全對（5.2s，正確排除已讀 / 時間戳），截圖→推薦端到端 16.2s（≤22s 預算內）；正式驗收 ⏳ 待團隊提供真實 LINE / Messenger 截圖各 10 張
- [ ] **P2-6** Console 主頁（React + Vite）：輸入區（含解析預覽編輯）、參數面板、結果卡片 + 👍👎、Debug 面板（05 文件 §2.1）— **L**（依賴 P2-4；UI 骨架可先行）
  - 驗收：05 文件 §5 全項
  - 進度：主頁全功能已實作 ✅（氣泡編輯器 + 範例載入 + 貼上解析、參數面板全參數 + /meta 梗圖包、結果卡片含分數拆解 / 策略徽章 / 👍👎+備註、Debug 面板含意圖 / 候選池表格 / 耗時、雷達掃描 loading、空結果診斷、敏感情境警示）；lib 層 vitest 6 項 + production build 通過；經 Vite 代理的完整 E2E（recommend 13.4s / 圖片 / 回饋落庫）通過 ✅；視覺與操作流暢度驗收 ⏳ 待團隊開 http://localhost:5173 確認；截圖上傳流程已隨 P2-5 接通 ✅
- [x] **P2-7** Console 次頁：查詢歷史（含重放）、梗圖庫瀏覽 + 手動上傳 — **M**（依賴 P2-6）
  - 驗收 ✅：查詢歷史（時間 / 輸入摘要 / 參數 / 👍👎 統計 / 重放載回工作台自動重跑）、梗圖庫（franchise / 分類 / 情緒篩選、單圖完整標註詳情、狀態標示）、手動上傳一條龍（匯入 → 標註 → 向量化，實測 7s 完成即可檢索；重複 409）；後端 +14 測試（repository 查詢 / 匯入重構 / API 契約），真實 E2E 通過；頁面視覺同 P2-6 待團隊瀏覽器確認
...
- [x] **P4-1** 回饋報表頁：👍 率趨勢、按參數 / 策略 / franchise 分組、👎 備註歸因五類（06 文件 §3.6）— **M**（依賴 P2-7）
