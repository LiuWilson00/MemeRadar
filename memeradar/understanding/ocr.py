...
def _media_type(image_bytes: bytes) -> str:
    """依 magic bytes 粗略判斷 png / jpeg（OCR 僅接受這兩種）。"""
    if image_bytes[:3] == b"\xff\xd8\xff":
        return "image/jpeg"
    return "image/png"


def _downscale_if_large(
    image_bytes: bytes, *, max_pixels: int = 6_000_000, max_side: int = 4096
) -> bytes:
    """圖片超過 max_pixels 或 max_side 就縮小並重壓 JPEG；已夠小 / 非圖片則原樣回。

    防呆：手機原始像素的高解析圖(20MP+)會拖慢 OCR、撐爆 gateway；前端已先縮，
    這裡是後端防線（直接打 API 的大圖也擋得住）。JPEG 用 draft 高效降解析。
    """
    try:
        ...
        if w * h <= max_pixels and max(w, h) <= max_side:
            return image_bytes
        scale = min((max_pixels / (w * h)) ** 0.5, max_side / max(w, h))
        nw, nh = max(1, int(w * scale)), max(1, int(h * scale))
        img.draft("RGB", (nw, nh))  # JPEG 解碼時即降解析，省記憶體
        img = img.convert("RGB").resize((nw, nh))
        out = io.BytesIO()
        img.save(out, format="JPEG", quality=85)
        return out.getvalue()
    except Exception:  # noqa: BLE001 縮圖失敗不擋主流程（大不了維持原圖交給 OCR）
        return image_bytes
...
class NvidiaOcr:
    ...
    def ocr(self, image_bytes: bytes) -> str:
        image_bytes = _downscale_if_large(image_bytes)  # 大圖先縮，免拖慢/撐爆
